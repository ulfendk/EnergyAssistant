    ...
    public async Task<SpotPrice[]> GetSpotPricesAsync()
    {
        try
        {
            ...
            var spotPrices = result.Data.Rows.Take(24).Select((x, i) =>
            {
                ...
                var hour = new DateTimeOffset(now.Year, now.Month, now.Day, now.Hour, 0, 0, now.Offset).AddHours(i);
                return new SpotPrice
                {
                    Hour = hour,
                    Region = (column.Name?.ToString() ?? throw new InvalidDataException(nameof(Column.Name))).ToLowerInvariant(),
                    IsPrediction = false,
                    Source = "Nordpool",
                    RawPrice = price
                };
